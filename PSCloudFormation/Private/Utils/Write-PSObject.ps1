...
Function Write-PSObject
{
    ...
    $lines = (($input | Format-Table -A | Out-String) -replace "`r", "" -split "`n") | Where-Object { $_.Trim() -ne "" };
    If (!($lines))
    {
        $lines = (($Object | Format-Table -A | Out-String) -replace "`r", "" -split "`n") | Where-Object { $_.Trim() -ne "" };
    }
    else
    {
        $Object = $input;
